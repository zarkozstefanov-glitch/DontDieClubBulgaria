...
export default function HeroNotifications() {
  ...
  useEffect(() => {
    ...
    if (visibleCount === 0) {
      const timer = setTimeout(() => setVisibleCount(1), 1500);
  ...
  return (
    <div className="mt-3 flex flex-col gap-1.5 min-[390px]:mt-4 min-[390px]:gap-2 min-[414px]:mt-5 min-[414px]:gap-3 min-[430px]:mt-6 min-[430px]:gap-3.5 md:hidden">
      <AnimatePresence mode="popLayout">
        {notifications.slice(0, visibleCount).map((item) => (
          <motion.div
            ...
            className="relative rounded-2xl border border-white/12 bg-white/10 p-2.5 text-white shadow-[0_10px_22px_rgba(0,0,0,0.1)] backdrop-blur-2xl min-[390px]:p-3 min-[414px]:rounded-3xl min-[414px]:p-4 min-[430px]:p-5"
          >
            <div className="mb-1.5 flex items-center gap-2 min-[390px]:mb-2 min-[390px]:gap-3">
              <div className="flex h-7 w-7 flex-shrink-0 items-center justify-center overflow-hidden rounded-lg border border-white/15 bg-white/15 shadow-sm min-[390px]:h-8 min-[390px]:w-8 min-[414px]:h-10 min-[414px]:w-10 min-[414px]:rounded-xl min-[430px]:h-11 min-[430px]:w-11">
            ...
            <p className="max-w-[92%] font-heading text-[11px] font-bold leading-snug text-white min-[390px]:text-xs min-[414px]:text-[15px] min-[430px]:text-[15px]">{item.title}</p>
            <p className="mt-0.5 text-[10px] leading-relaxed text-white/70 min-[390px]:text-[11px] min-[414px]:text-[13px] min-[430px]:text-sm">{item.label}</p>
          </motion.div>
        ))}
      </AnimatePresence>
